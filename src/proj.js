...
function procJson(params, filename) {
    let bufjson = fs.readFileSync(path.join(__dirname, '../template', filename), 'utf-8');
    ...
    for (let ii = 0; ii < objout.length; ++ii) {
        ...
        if (cur.type == 'dir') {
            ...
            let srcfilename = path.join(__dirname, '../template', cur.src);
...
function procProj(params) {
    ...
    let objJson = procJson(params, 'main.json');
